...
(async()=>{
 ...
 try{
  ...
  for(const r of results){assert.strictEqual(r.error,0,JSON.stringify(r));r.pixel.forEach((v,i)=>assert(Math.abs(v-r.expected[i])<=1,JSON.stringify(r)));}
  console.log('WebGL directional lighting PASS '+results.length+' pixel cases');
