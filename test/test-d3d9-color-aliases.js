...
const sigs=require('../lib/host-import-sigs.generated.json').sigs,cases=require('./fixtures/d3d9-color-alias-cases');
(async()=>{
 ...
 const consumer=new WorkerConsumer(worker,{module,memory,sigs,imageBase:0x400000,sourceVersion:'alias-test',reclaimHeap:head=>e.d3d_render_adopt_free_list(head)});
 try{await consumer.ready;
  for(const [name,queue]of[['direct',new CommandQueue({deviceId:1,consumer:{execute:c=>direct.execute(c)}})],['worker',new CommandQueue({deviceId:2,consumer})]]){
   const send=async(op,p)=>{const r=queue.submit(op,p),v=await r.value;if(v?.error)throw Error(v.error.message||v.error);return v;};
   if(name==='worker')await send(O.RESOURCE_CREATE,{kind:'device',width:8,height:8});
   console.log(name+' color aliases PASS '+await cases.run({create:resource=>send(O.RESOURCE_CREATE,{kind:'color',resource}),
    update:(resource,pixels,pitch)=>send(O.RESOURCE_UPDATE,{kind:'color',resource,pixels,pitch}),
    ...
    draw:p=>send(O.DRAW,p),read:resource=>send(O.READBACK,{resource}),release:id=>send(O.RESOURCE_RELEASE,{kind:'color-set',ids:[id]})}));
   await send(O.RESOURCE_RELEASE,{kind:'device'});
  }
 }finally{direct.destroy();await consumer.cancel();assert.strictEqual(worker.threadId,-1);assert(Number.isInteger(consumer.shutdownInfo.heapAdopted));}
