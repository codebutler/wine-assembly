...
const viewportSource = fs.readFileSync(path.join(root, 'lib', 'page-viewport.js'), 'utf8');
...
assert(viewportSource.includes('async function approveBrowserFullscreen(event)'),
  'the viewport controller should own the fullscreen request');
assert(viewportSource.includes('const request = target.requestFullscreen || target.webkitRequestFullscreen'),
  'approval should support the browser fullscreen API');
...
assert(viewportSource.includes('if (!request) return enterPageFullscreen();'),
  'a browser without the fullscreen API should fall back to the whole page');
assert(viewportSource.includes('function enterPageFullscreen()') &&
  viewportSource.includes('function exitPageFullscreen(event)'),
...
const exitBody = viewportSource.slice(
  viewportSource.indexOf('function exitPageFullscreen(event)'),
  viewportSource.indexOf('function exitPageFullscreenFromPinch()'));
assert(exitBody.includes('if (event) currentRenderer._fullscreenDeclined = true;'),
  'only a user-initiated exit should latch a fullscreen decline');
assert(!/\n\s*currentRenderer\._fullscreenDeclined = true;/.test(exitBody),
...
assert(!viewportSource.includes('if (renderer._exclusiveFullscreen) requestBrowserFullscreen()'),
  'canvas clicks and touches must not retry browser fullscreen');
assert(!html.includes("app.args === '/s' && !document.fullscreenElement"),
  'screensavers must not force browser fullscreen at launch');
