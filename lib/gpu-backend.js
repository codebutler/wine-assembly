  ...
  class WebGLBackend {
    ...
    bindRenderTarget(key, width, height, depthBits = 0, logicalWidth=this.canvas.width, logicalHeight=this.canvas.height) {
      ...
      if (!target) {
        ...
        const bytes = width * height * (depthBits ? 8 : 4);
        ...
        try {
          ...
          if (depthBits) {
            ...
            if (!target.depth) throw new Error('Depth allocation failed');
            gl.bindRenderbuffer(gl.RENDERBUFFER,target.depth);
            gl.renderbufferStorage(gl.RENDERBUFFER,depthBits===16?gl.DEPTH_COMPONENT16:
              this.version===2?gl.DEPTH24_STENCIL8:gl.DEPTH_STENCIL,width,height);
            if (gl.getRenderbufferParameter(gl.RENDERBUFFER,gl.RENDERBUFFER_DEPTH_SIZE)!==depthBits)
              throw new Error('Requested depth precision unavailable');
            if(depthBits===24&&gl.getRenderbufferParameter(gl.RENDERBUFFER,gl.RENDERBUFFER_STENCIL_SIZE)!==8)
              throw new Error('Requested stencil precision unavailable');
          ...
          this.clear([0,0,0,0],gl.COLOR_BUFFER_BIT|(depthBits?gl.DEPTH_BUFFER_BIT:0)|(depthBits===24?gl.STENCIL_BUFFER_BIT:0));
    ...
    resetRenderTargets(width,height,depth) {
      ...
      this._targets=new Map(); this._target=null;
      try {
        this.bindRenderTarget(depth?.key??null,depth?.width??width,depth?.height??height,depth?.bits??0,width,height);
      } catch(error) {
        this._targets=oldMap; this._target=oldTarget; this._targetBytes=oldBytes;
