...
function loadDll(exports, memory, dllBytes, modulePath) {
  ...
  let loadAddr = exports.get_next_dll_addr();
  const dllMain = exports.load_dll(dllBytes.length, loadAddr);
  // load_dll rebases an image that would not fit in the guest's low spans
  // into a sparse reservation; its DLL_TABLE row holds where it really went.
  const loadedCount = exports.get_dll_count ? exports.get_dll_count() >>> 0 : 0;
  if (loadedCount > dllCount && exports.get_dll_table) {
    loadAddr = new DataView(memory).getUint32(exports.get_dll_table() + (loadedCount - 1) * 32, true);
  }
...
// sitting next to the exe — see $win16_win87em. TOOLHELP, AVIFILE and MSVIDEO
// are answered in WAT under their dynamic ids ($win16_toolhelp, $win16_avifile,
// $win16_msvideo), so there is no DLL to go looking for.
// DISPLAY is the display *driver*, and on a real machine it is part of the
// installation rather than a file beside the app, so there is never anything
// to stage for it. WING.DLL imports DISPLAY.#1 (the driver's own BitBlt) for
// its fast path, and with the name unresolved the whole task dies at load
// before deciding whether it wants that path at all. Named here, the import
// resolves to a thunk and the Win16 dispatcher's fail-fast tail reports
// DISPLAY.<ordinal> if the driver entry is genuinely called.
const WIN16_EMULATED_MODULES = new Set(['DDEML', 'WIN87EM', 'VER', 'TOOLHELP', 'AVIFILE', 'MSVIDEO', 'DISPLAY']);
...
function loadWin16Dlls(exports, memory, exeBytes, dir, readFile, log = () => {}, extraNames = []) {
  ...
  const staged = new Map();   // module id -> file bytes
  const imports = [];         // the imported modules that loaded, as {name, id}
  const stage = (name, id) => {
  ...
  for (const [name, id] of Object.entries(WIN16_MODULE_IDS)) {
    ...
    if (exports.load_ne_dll(id)) { loaded.push(name); imports.push({ name, id }); log(`[win16] loaded ${name}`); }
  ...
  if (exports.win16_dynamic_module_slot) {
    ...
    for (let i = 0; i < WIN16_DYNAMIC_MODULES; i++) {
      ...
      if (exports.load_ne_dll(WIN16_DYNAMIC_BASE + i)) {
        loaded.push(name);
        imports.push({ name, id: WIN16_DYNAMIC_BASE + i });
  ...
  queueWin16DllInits(exports, imports, staged);
  return loaded;
}

// Windows runs the LibEntry of every DLL a task imports before the task's own
// entry point, and a DLL's own imports before it. Queue the loaded ones in that
// order; WAT runs them on the task's stack and then starts the task (see
// $win16_begin_dll_inits).
function queueWin16DllInits(exports, imports, staged) {
  if (!exports.win16_queue_dll_init || !exports.win16_begin_dll_inits || !imports.length) return;
  const byName = new Map(imports.map(m => [m.name.toUpperCase(), m]));
  const seen = new Set();
  const visit = (m) => {
    if (seen.has(m.id)) return;
    seen.add(m.id);
    for (const dep of win16ModuleNames(staged.get(m.id) || new Uint8Array(0))) {
      const d = byName.get(dep.toUpperCase());
      if (d) visit(d);
    }
    exports.win16_queue_dll_init(m.id);
  };
  imports.forEach(visit);
  exports.win16_begin_dll_inits();
}
