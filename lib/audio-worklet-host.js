...
function createWorkletRouter(options = {}) {
  ...
  const router = {
    ...
    // Is the feature switched on at all? Default OFF: this is an opt-in path
    // and the buffer-source splice remains what ships until it is measured on
    ...
    enabled(ctx) {
      ...
      if (typeof location !== 'undefined' && location.search) {
        return /[?&]audio-worklet\b/.test(location.search);
      }
      return false;
    },
