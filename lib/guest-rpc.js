    ...
    names.forEach((name, id) => {
      ...
      if (name === 'gpu_gl_call' && glEncoder) {
        host[name] = (opcode, stackWa, aux) => {
          ...
          if (opcode >= 0x30000 && opcode <= 0x30016) return brokerCall(id, [opcode, stackWa, aux], false);
          if (opts.d3dCommands) {
    ...
    return {
      ...
      serveGlBatch(msg) {
        ...
        try {
          ...
          const batch = msg.batch || (msg.memoryOffset !== undefined
            ? { memoryOffset: msg.memoryOffset >>> 0, bytes: msg.bytes >>> 0 }
            : null);
          if (typeof fn === 'function') result = fn(batch, slot) | 0;
