 ...
 outline_guest();
  ...
  class Device {
    ...
    createColor(resource,pixels=null,pitch=resource.width*4){
      ...
      this.colorTargets.set(resource.id,{...resource});return 1;
    }
    updateColor(resource,pixels,pitch=resource.width*4){if(!this.colorResource(resource))invalid('unknown color resource');
      this.gpu.updateColorResource(resource.id,this.colorPixels(resource,pixels,pitch));return 1;}
    readColor(resource){
    ...
    releaseColor(id){this.gpu.releaseColorResource(id);this.colorTargets.delete(id);return 1;}
    ...
    destroy() { this.gpu.destroy(); this.programs.clear(); this.outlinePrograms.clear(); this.textures.clear(); this.textureTargets.clear(); this.depthTargets.clear(); this.colorTargets.clear(); this.mipUniforms?.clear(); }
  }
