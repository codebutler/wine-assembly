...
void main(){
 ...
 outline_guest();
  ...
  class Device {
    ...
    bindDepth(metadata) {
      const d=this.depthTarget(metadata);
      ...
      this.gpu.bindRenderTarget(d.key,d.width,d.height,d.bits);
    ...
    clear(color, flags, depth=1, rects=null,depthAttachment=undefined,stencil=0) {
      ...
      const regions=rects===null?[[0,0,g.canvas.width,g.canvas.height]]:rects;
      if(!Array.isArray(regions)||regions.length>65536||regions.some(r=>!Array.isArray(r)||r.length!==4||
        r.some(v=>!Number.isInteger(v))||r[0]<0||r[1]<0||r[2]<r[0]||r[3]<r[1]||r[2]>g.canvas.width||r[3]>g.canvas.height))invalid('invalid clear rectangles');
      this.bindDepth(depthAttachment);
      g.setDepthMask(true);
    ...
    draw(draw) {
      ...
      if(scissor&&(![true,false,0,1].includes(scissor.enabled)||(scissor.enabled&&(
        ...
        scissor.right>g.canvas.width||scissor.bottom>g.canvas.height))))invalid('invalid scissor rectangle');
      ...
      this.bindDepth(draw.depthAttachment);
      ...
      for (const shader of [vs, ps]) for (const name of shader.uniforms) {
        ...
        if(name==='d3d_alpha_ref'){
          ...
          this.uploadTexture(stage, texture,ps.mipStages?.includes(stage));
      ...
      gl.colorMask(!!(colorMask&1),!!(colorMask&2),!!(colorMask&4),!!(colorMask&8));
      ...
      if(outline){
        ...
        g.setUniform(program,'outline_target','4f',[g.canvas.width,g.renderHeight,vp.minZ,vp.maxZ]);
        ...
        g.setCapability(gl.CULL_FACE,false);g.setViewport(0,0,g.canvas.width,g.renderHeight);
    ...
    uploadTexture(stage, image, atlas = false) {
      if(atlas)return this.uploadMipAtlas(stage,image);
      const g = this.gpu, gl = g.gl;
      ...
      chains.forEach((levels, face) => levels.forEach((level,index) => {
    ...
    uploadMipAtlas(stage,image) {
      ...
      levels.forEach((level,i)=>{
        if(level.width!==width||level.height!==height||!(level.pixels instanceof Uint8Array)||level.pixels.length!==width*height*4)
    ...
    destroy() { this.gpu.destroy(); this.programs.clear(); this.outlinePrograms.clear(); this.textures.clear(); this.textureTargets.clear(); this.depthTargets.clear(); this.mipUniforms?.clear(); }
  }
