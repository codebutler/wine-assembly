...
(function(root, factory) {
  ...
  class Device {
    ...
    prepare(snapshot, deferred=false) {
      ...
      let batches;
      try{batches=large?Geometry.split(snapshot,this.budget-this.bytes-snapshotBytes).batches:[snapshot];}
    ...
    prepareBatch(snapshot,draw) {
      ...
      try {
        ...
        const nativeBytes=this.exports()[draw.setup?'d3d_software_deferred_allocation_bound':'d3d_software_allocation_bound'](nativeIndices?n:count,count)>>>0;
        ...
        if(draw.setup)draw.context=this.exports().d3d_software_create_deferred(desc.wa,draw.typedConstants)>>>0;
