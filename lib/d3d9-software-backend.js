...
(function(root, factory) {
  ...
  class Device {
    ...
    prepareBatch(snapshot,draw) {
      ...
      try {
        ...
        if(scissor&&(![true,false,0,1].includes(scissor.enabled)||(scissor.enabled&&(
          ...
          scissor.right>this.width||scissor.bottom>this.height))))invalid('invalid scissor rectangle');
    ...
    destroy(){if(this.destroyed)return;this.cancel();this.bytes-=this.queries.size*32;this.queries.clear();this.freeFixedCache();for(const surface of this.depthSurfaces.values())this.free(surface);this.depthSurfaces.clear();this.free(this.target);this.free(this.depth);this.destroyed=true;}
    execute(command){
