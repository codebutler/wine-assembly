...
(function(root, factory) {
  ...
  class Device {
    ...
    fixedPrograms(snapshot, viewport, draw, fixedVS=true, fixedPS=true, uvRequired=0) {
      ...
      const unsupported=['lighting','specular'].filter(flag=>f[flag]&&(flag!=='lighting'||fixedVS));
    ...
    prepareBatch(snapshot,draw) {
      ...
      try {
        ...
        // A generated coordinate frees at least one UV slot. Reuse that input
        // lane for NORMAL without changing the raster descriptor or register ABI.
        for(const input of extraInputs){const slot=selected.findIndex((a,i)=>i>=2&&i<8&&!a);
          if(slot<0)invalid('NORMAL/specular plus independent UV inputs exceeds native input slots');
          selected[slot]=input.a;registers[slot]=input.register;uvCount=Math.max(uvCount,slot-1);}
        ...
        const hasPsize=!!selected[8],slots=2+uvCount+(hasPsize?1:0),packedStride=slots*16;
        if(hasPsize){selected[2+uvCount]=selected[8];registers[2+uvCount]=registers[8];}
        ...
        const extended=uvCount>1||hasPsize;
        let extraMap=0;for(let i=3;i<2+uvCount;i++)extraMap|=registers[i]<<((i-3)*4);
        if(hasPsize)extraMap|=registers[2+uvCount]<<20;
        const depth=this.depthSurface(snapshot.depthAttachment);
        u32.set([0x44535031,hasPsize?4:uvCount>4?3:extended?2:1,this.width,this.height,this.target.wa,this.pitch,depth?depth.wa:0,depth?depth.pitch:0,
        ...
        u32.set([(depth&&state.zenable?(1|(state.zwrite?2:0)):0)|(pretransformed?4:0)|(fillMode===1?8:0),state.zfunc===undefined?4:state.zfunc,writeMask,
          state.cull===undefined?1:state.cull,registers[0]|registers[1]<<4|registers[2]<<8,extended?uvCount:0,extraMap],25);
