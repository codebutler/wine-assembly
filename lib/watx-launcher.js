...
(function (root, factory) {
  ...
  async function fetchSources(options = {}) {
    ...
    // Dispatch replication (lib/dispatch-replicate.js), the same rewrite
    // tools/watx-closure.js applies to the build's closure, so a source
    // compile here yields the module the shipped artifact is. It is the one
    // place this path decodes a source to text: only the files holding a
    // `(return_call $next)` are decoded, rewritten and re-encoded, and the
    // cache key is taken over the rewritten bytes, so a shared-dispatch
    // snapshot and a replicated one never share a key.
    if (dispatchMode(options) === 'replicated') {
      const lib = dispatchReplicateLib();
      const decoded = {};
      lib.replicateSources(manifest,
        name => { if (!(name in decoded)) decoded[name] = toText(sources[name]); return decoded[name]; },
        (name, text) => { sources[name] = new TextEncoder().encode(text); });
    }
  ...
  // 'replicated' (default) or 'shared': options.dispatch, else WINE_DISPATCH
  // in node, else the browser page's ?dispatch= query.
  function dispatchMode(options) {
    let raw = options.dispatch;
    if (raw == null && isNode) raw = process.env.WINE_DISPATCH;
    if (raw == null && typeof location !== 'undefined' && location.search) {
      raw = new URLSearchParams(location.search).get('dispatch');
    }
    const value = raw == null ? 'replicated' : String(raw).trim();
    if (value === 'replicated' || value === 'all' || value === '1' || value === 'true' || value === '') return 'replicated';
    if (value === 'shared' || value === 'none' || value === '0' || value === 'false') return 'shared';
    throw new Error('watx-launcher: dispatch must be "replicated" or "shared", got ' + JSON.stringify(raw));
  }

  function dispatchReplicateLib() {
    if (isNode) return require('./dispatch-replicate.js');
    const lib = typeof self !== 'undefined' ? self.DispatchReplicate : undefined;
    if (!lib) throw new Error('watx-launcher: lib/dispatch-replicate.js must be loaded before lib/watx-launcher.js');
    return lib;
  }
