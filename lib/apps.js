...
//                   Fill mode ONLY: the default view is 'fit', so a crop is a
//                   thing the player opts into, never the framing they land on.
//   mobileZoom    — how many phone pixels one guest pixel is worth. The guest
//                   desktop is the viewport divided by this on BOTH axes, and
//                   the single-app fit scales it back out, so the picture is
//                   magnified by exactly this factor with no gutters and no
//                   crop. For an app that draws at a fixed pixel scale, where a
//                   bigger desktop means more playfield rather than a bigger
//                   picture. See Win98Renderer.singleAppBackingSize.
  ...
  const DESKTOP_APPS = [
    ...
    const APPS = {
      ...
      ski32: {
        ...
        // desktop buys more snow, never a bigger skier. Measured in landscape
        // on a 667x375 CSS viewport: the maximized window is the desktop, the
        // fit is 1:1, and the skier lands 31 CSS px wide, about 5mm on the
        // device against 11mm for the same sprite on a 96dpi monitor.
        //
        // 1.5 hands the guest 445x250 instead and the fit scales it back out:
        // a 46 CSS px skier, two thirds of the way back to desktop size, and
        // still enough slope below him to react to (he starts ~31% down the
        // window whatever its height, so the look-ahead is ~172 guest rows).
        // 2x was measurably too far — at 334x188 the three start signs collide
        // with the control hints and the reaction distance halves.
        mobileZoom: 1.5,
      ...
      pinball: {
        ...
        // width nobody is short of. One framing means no chip to toggle.
        ...
        touchControls: {
          ...
          buttons: [
            ...
            { vk: 0xBE, icon: 'arrow-right', title: 'Nudge right',
              pos: 'board-tr' },
            // ...which empties the bottom-left rail, so the one action button
            // comes down off the picture and into it. (It was top-LEFT because
            // the page-fullscreen exit chip and the debug peek own the
            // top-right; down here it is clear of both and of the table.)
      ...
      black_white_2_demo: {
        ...
        // for more; measured, not guessed (docs/re-notes/black-white-2.md).
        // This is the only app in the registry that asks for the 1GB memory.
        bigMemory: true,
      },
      quake2_demo: {
      ...
      funtris:    {
        ...
        // it is a small part of the window: caption, menu bar and a tall strip
        // of empty client grey are the rest. Without a crop, Fill takes the
        // window's centre and trims it to the phone's aspect, which slices the
        // panel off one side and the well off the other -- a fill that cuts
        // the game.
        //
