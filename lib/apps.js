  ...
  const DESKTOP_APPS = [
    ...
    const APPS = {
      ...
      ski32: {
        ...
        //
        // 1.5 hands landscape a 445x250 desktop instead of 667x375 and the fit
        // scales it back out: a 46 CSS px skier, two thirds of the way back to
        // the ~11mm the same sprite has on a 96dpi monitor, with enough slope
        // left below him to react to (he starts ~31% down the window whatever
        // its height, so the look-ahead is ~172 guest rows). 2x was measurably
        // too far — at 334x188 the three start signs collide with the control
        // hints and the reaction distance halves.
        mobileZoom: { landscape: 1.5 },
        ...
        touchControls: {
          ...
          buttons: [
            ...
            { vk: 0x58, icon: 'arrow-left', title: 'Nudge left',
              pos: 'board-tl', fit: { cx: 0.0886, cy: 0.0767, r: 0.0885 } },
            { vk: 0xBE, icon: 'arrow-right', title: 'Nudge right',
              pos: 'board-tr', fit: { cx: 0.0886, cy: 0.0767, r: 0.0885 } },
