...
(function (root, factory) {
  ...
  const VS20_OPS = {32:['pow',3],33:['crs',3],34:['sgn',4],35:['abs',2],36:['nrm',2],37:['sincos',4],38:['rep',1],39:['endrep',0],40:['if',1],42:['else',0],43:['endif',0],46:['mova',2],47:['defb',2],48:['defi',5]};
  ...
  function compileIR(shader, options = {}) {
    ...
    const boolInputs=new Set(),intInputs=new Set(),flowStack=[];let staticFlowCount=0,repSerial=0;
    ...
    for (const ins of shader.instructions) {
      ...
      if(vs20&&[38,39,40,42,43].includes(ins.opcode)){
        if(coissue)fail('invalid coissued flow instruction',current);
        if(ins.opcode===38){
          const n=d&2047;
          if(n>=16||(d>>>0)!==((0xf0e40000|n)>>>0))fail('invalid REP integer source',current);
          if(flowStack.some(frame=>frame.kind==='rep'))fail('nested REP exceeds VS2 limit',current);
          if(++staticFlowCount>16)fail('static flow count exceeded',current);
          const counter=`d3d_rep${repSerial++}`;flowStack.push({kind:'rep'});intInputs.add(n);
        ...
        }else if(ins.opcode===39){
          if(flowStack[flowStack.length-1]?.kind!=='rep')fail('unmatched or straddling ENDREP',current);
          ...
          if(++staticFlowCount>16)fail('static flow count exceeded',current);
          flowStack.push({kind:'if',hasElse:false});boolInputs.add(n);lines.push(`if (d3d_vs_b${n}) {`);
          ...
          if(++staticFlowCount>16)fail('static flow count exceeded',current);
          flowStack[flowStack.length-1].hasElse=true;lines.push('} else {');
        }else{
    ...
    integerUniformRanges:Array.from(intInputs).filter(n=>!intConstants.has(n)).map(n=>({name:`d3d_vs_i${n}`,component:0,min:0,max:255})),
