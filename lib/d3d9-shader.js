...
(function (root, factory) {
  ...
  const VS20_OPS = {27:['loop',2],29:['endloop',0],32:['pow',3],33:['crs',3],34:['sgn',4],35:['abs',2],36:['nrm',2],37:['sincos',4],38:['rep',1],39:['endrep',0],40:['if',1],42:['else',0],43:['endif',0],46:['mova',2],47:['defb',2],48:['defi',5]};
  ...
  function compileIR(shader, options = {}) {
    ...
    const boolInputs=new Set(),intInputs=new Set(),loopInputs=new Set(),flowStack=[];let staticFlowCount=0,repSerial=0,currentRelativeBank='a0';
    ...
    function reg(token, write = false) {
      ...
      if (type === 2 && n < (pixel ? 8 : vertexConstants) && !write) {
        if(token&0x2000) {
          ...
          if(currentRelativeBank==='aL'&&!loopFrame)fail('aL relative address outside LOOP',current);
          if(currentRelativeBank!=='aL'&&!addressWritten)fail('relative address used before a0 initialization',current);
          relative=true;
          for(let i=0;i<vertexConstants;++i)constants.set(i,null);
          return `(d3d_relative(${currentRelativeBank==='aL'?`float(${loopFrame.address})`:'a0.x'} + ${n}.0))`;
        }
    ...
    for (const ins of shader.instructions) {
      ...
      if(addressBanks.some(x=>x!=='a0'&&x!=='aL')||new Set(addressBanks).size>1)
      ...
      if(vs20&&[27,29,38,39,40,42,43].includes(ins.opcode)){
        ...
        if(ins.opcode===38||ins.opcode===27){
          ...
          flowStack.push({kind:isLoop?'loop':'rep',address:`(d3d_vs_i${n}.y + ${counter} * d3d_vs_i${n}.z)`});intInputs.add(n);
