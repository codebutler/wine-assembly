...
// The system's stock icons (LoadIcon(NULL, IDI_*)): Wine's own, see
// icons/system/UPSTREAM.md. Every host mounts them where the engine reads them
// the first time one is drawn; a host without them keeps icon-less stock
// handles, as before.
const STOCK_ICON_FILES = [
  'oic_sample', 'oic_hand', 'oic_ques', 'oic_bang', 'oic_note', 'oic_winlogo',
].map(name => ({ url: `icons/system/${name}.ico`, vfsPath: `c:\\windows\\system\\${name}.ico` }));
...
const processBootApi = {
  SYSTEM_DATA_FILES, STOCK_ICON_FILES, resolveDllGraph, mountLoadedDllFiles, mountSystemDataFiles,
