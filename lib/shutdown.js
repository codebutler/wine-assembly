...
(function () {
  ...
  const STYLE = `
    ...
    /* index.html styles every <canvas> as the app display (absolute at 0,0,
       ...
       NOTE: this selector is only (1,1,1) and several of those full-bleed
       rules outrank it -- body.page-fullscreen canvas:not(#touch-cursor) is
       (1,1,2) and the .exclusive-fullscreen pair are (1,2,2), all three
       setting height:0 !important. What keeps the logo from collapsing is
       not this rule, it is fit(): it sets width and height as INLINE
       !important declarations, which outrank every stylesheet rule whatever
       its specificity. Verified in a real browser by test-web-shutdown, which
       measures the logo's box inside element fullscreen. So if fit() is ever
       changed to set plain inline styles, this rule does not catch it -- the
       size is the inline declaration's job alone. (The FPS HUD had no such
       backstop and did collapse; see lib/perf-hud.js.) */
