...
class Win98Renderer {
  constructor(canvas) {
    ...
    // Registry `mobileZoom`: how many phone pixels one guest pixel is worth.
    // 0/1 is the default 1:1. See singleAppBackingSize.
    this.singleAppZoom = 0;
  ...
  singleAppBackingSize(width, height) {
    ...
    const zoom = +this.singleAppZoom || 0;
    if (zoom > 1) {
  ...
  _computeSingleAppZoom(topLevels) {
    ...
    if (contain) {
      ...
      if (!zoomMode && outW > outH && source.w / source.h < outW / outH &&
      ...
      const grown = this._growToAspect(source, bounds, full);
      ...
      if (!fills && area.h < 0.9 && area.w > 0.9 &&
        ...
        source = fills ? grown : this._growToAspect(source, bounds, area);
      }
    }
    const transform = this._computeExclusiveTransform(source);
    if (!transform) return null;
