...
class Win98Renderer {
  ...
  showWindow(hwnd, cmd) {
    ...
    if (win.visible) {
      ...
      if (!wasVisible && !win.isChild) this._markLaunchPaint('show_window');
      this.invalidate(hwnd);
    }
  }

  _markLaunchPaint(name) {
    const timing = this._launchTiming ||
      (typeof window !== 'undefined' ? window.__waLaunchTiming : null);
    if (!timing || !timing.mark || timing.complete) return;
    // Ignore desktop/shared repaints that happen before the guest run loop starts.
    if ((name === 'show_window' || name === 'first_composite') &&
        timing.marks.run_start == null) {
      return;
    }
    timing.mark(name);
    if (name === 'first_composite') {
      timing.complete = true;
      if (typeof console !== 'undefined' && console.log) {
        const s = timing.summary ? timing.summary() : timing;
        console.log('[launch-timing]', JSON.stringify(s));
      }
    }
  }
  ...
  _repaintOnce() {
    ...
    if (visibleTopLevel.length) this._markLaunchPaint('first_composite');
