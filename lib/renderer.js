...
class Win98Renderer {
  ...
  _menuPaintBar(win, x, y, w) {
    ...
    this._ensureWatMenu(win);
    // The highlighted item is the open dropdown's bar POSITION; once a bar
    // has items of its own (MDI's system menu and buttons) that is not the
    // dropdown's content index, menu_open_top.
    const openIdx = (e.menu_open_hwnd && e.menu_open_hwnd() === win.hwnd)
      ? ((e.menu_open_bar_pos ? e.menu_open_bar_pos() : e.menu_open_top()) | 0) : -1;
    const wc = this.getWindowCanvas(win.hwnd);
    if (!wc) return 0;
  ...
  _openMenuGeometry() {
    ...
    const barPos = e.menu_open_bar_pos ? (e.menu_open_bar_pos() | 0) : top;
    const dx = explicitX >= 0 ? explicitX : pos.barX + (e.menu_bar_item_x(hwnd, barPos) | 0);
