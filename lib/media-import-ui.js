...
.wa-media-import-btn{position:absolute;left:8px;bottom:8px;z-index:1;font:12px system-ui,sans-serif;
  padding:4px 10px;background:#c0c0c0;border:2px outset #fff;cursor:pointer}
/* Desktop furniture: it shares the desktop icons' layer (z-index 1, under the
   guest's windows on the screen canvas) and their visibility rules. A running
   single-app page and an exclusive-fullscreen surface both hide the desktop,
   so the affordance goes with it. Only the no-debug canvas has a z-index (2);
   the ?debug canvas stacks at auto, below z-index 1, so there the button
   hides while a guest runs, the way the icon grid does. */
body.single-app.app-running .wa-media-import-btn,
body:not(.no-debug).app-running .wa-media-import-btn,
