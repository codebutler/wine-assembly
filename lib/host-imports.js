...
function createHostImports(ctx) {
  ...
  const _flushGLCommands = exports => {
    let result = _glCommands ? _glCommands.flush() : 0;
    const e = exports || ctx.exports || (ctx.instance && ctx.instance.exports);
    if (e && e.gl_wat_encoder_enabled && e.gl_wat_encoder_enabled()
        && e.gl_wat_stream_flush) result = e.gl_wat_stream_flush() | 0;
    return result | 0;
  };
  ...
  const host = {
    ...
    gpu_gl_call: (opcode, stackWa, aux) => {
      ...
      // The WAT encoder writes the same records as Encoder above directly into
    ...
    gpu_gl_batch: (batch, owner) => {
      ...
      // Worker WAT encoders hand off an offset into the already-shared memory;
      // legacy JS encoders still hand off their owned SharedArrayBuffer batch.
      const replayBatch = batch && batch.memoryOffset !== undefined && _glCommandStream
        ? _glCommandStream.memoryBatch(ctx.getMemory(), batch.memoryOffset, batch.bytes)
        : batch;
