...
function createHostImports(ctx) {
  ...
  const host = {
    ...
    gpu_gl_call: (opcode, stackWa, aux) => {
      ...
      // 0x20000-0x20003 is the D3DIM render-Worker protocol (draw, fence,
      // ready, flip). A host that attached an encoder -- the CLI's --d3d-worker --
      ...
      // OpenGL encoder.
      if ((opcode | 0) >= 0x20000 && (opcode | 0) <= 0x20003) {
