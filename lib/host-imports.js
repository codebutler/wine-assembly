...
function createHostImports(ctx) {
  ...
  // Effective mapping extents for a DC: {wx, wy} window (logical) extents and
  // {vx, vy} viewport (device) extents. Fixed-scale modes imply their extents
  // at 96 dpi with inverted y; MM_ISOTROPIC/ANISOTROPIC use the app-set
  // extents; MM_TEXT is identity.
  const _mapExtents = (dc) => {
    const DPI = 96;
    switch (dc.mapMode | 0) {
      case 2: return { wx: 254, wy: 254, vx: DPI, vy: -DPI };   // MM_LOMETRIC (0.1mm)
      case 3: return { wx: 2540, wy: 2540, vx: DPI, vy: -DPI }; // MM_HIMETRIC (0.01mm)
      case 4: return { wx: 100, wy: 100, vx: DPI, vy: -DPI };   // MM_LOENGLISH (0.01")
      case 5: return { wx: 1000, wy: 1000, vx: DPI, vy: -DPI }; // MM_HIENGLISH (0.001")
      case 6: return { wx: 1440, wy: 1440, vx: DPI, vy: -DPI }; // MM_TWIPS (1/1440")
      case 7: case 8:                                           // MM_ISOTROPIC/ANISOTROPIC
        return {
          wx: dc.appWinExtX || 1, wy: dc.appWinExtY || 1,
          vx: dc.appVpExtX || 1, vy: dc.appVpExtY || 1,
        };
      default: return { wx: 1, wy: 1, vx: 1, vy: 1 };           // MM_TEXT
    }
  };
  ...
  // Socket imports are an optional host integration. The standalone browser
  // and CLI hosts do not provide a transport, so expose Winsock-compatible
  // failure defaults; embedders such as pc replace these functions with their
  // own socket bridge after createHostImports() returns.
  let _sockLastError = 10050; // WSAENETDOWN
  const _sockUnavailable = () => {
    _sockLastError = 10050;
  ...
  const host = {
    ...
    // --- Optional Winsock transport ---
    sock_socket: _sockUnavailable,
    sock_connect: _sockUnavailable,
    sock_send: _sockUnavailable,
    sock_recv: _sockUnavailable,
    sock_close: _sockUnavailable,
    sock_gethostbyname: () => {
      _sockLastError = 11001; // WSAHOST_NOT_FOUND
      return 0;
    },
    sock_inet_addr: (ptr) => {
      const parts = readStr(ptr, 16).split('.');
      if (parts.length !== 4) return -1;
      const octets = parts.map(part => /^\d+$/.test(part) ? Number(part) : -1);
      if (octets.some(octet => octet < 0 || octet > 255)) return -1;
      return (octets[0] | (octets[1] << 8) | (octets[2] << 16) | (octets[3] << 24)) | 0;
    },
    sock_ioctl: _sockUnavailable,
    sock_select: _sockUnavailable,
    sock_last_error: () => _sockLastError,
    sock_api: (op, a0, a1, a2) => {
      if (op === 3) { // WSASetLastError
        _sockLastError = a0 | 0;
        return 0;
      }
      if (op === 15 && a2) { // WSAHton{s,l} / WSANtoh{s,l}
        const dv = new DataView(ctx.getMemory());
        if (a0 === 2) {
          const value = ((a1 & 0xFF) << 8) | ((a1 >>> 8) & 0xFF);
          dv.setUint16(a2, value, true);
          return 0;
        }
        if (a0 === 4) {
          const value = (((a1 & 0xFF) << 24) |
            ((a1 & 0xFF00) << 8) |
            ((a1 >>> 8) & 0xFF00) |
            ((a1 >>> 24) & 0xFF)) >>> 0;
          dv.setUint32(a2, value, true);
          return 0;
        }
      }
      _sockLastError = 10050;
    ...
    sock_async_select: _sockUnavailable,
    sock_post_event: () => {},
    ...
    help_open: (pathWA) => {
      ...
      // Prefer window.HlpParser (hlp-parser.js stamps it). Bare `HlpParser`
      // works in classic sloppy globals; require() is the Node test path.
      const HlpParserClass =
        (typeof window !== 'undefined' && window.HlpParser) ||
        (typeof HlpParser !== 'undefined' ? HlpParser : null) ||
      ...
      if (ctx.readFile) {
        ...
        if (data && data.length > 0) {
          try {
            ...
            if (parser.parse() && parser.topics.length > 0) {
    ...
    draw_text: (x, y, textPtr, textLen, color) => {
      if (!ctx.renderer) return;
      // Guest memory may be a SharedArrayBuffer (threads) — TextDecoder rejects
      // a shared-backed view, so copy first (same fix as winsock cstr / log).
      const bytes = new Uint8Array(ctx.getMemory()).slice(textPtr, textPtr + textLen);
    ...
    get_window_class: (hwnd, bufWA, maxLen) => {
      const win = ctx.renderer && ctx.renderer.windows && ctx.renderer.windows[hwnd];
      const text = (win && win.className) ? String(win.className) : '';
      if (!bufWA || maxLen <= 0) return text.length;
      const bytes = new Uint8Array(ctx.getMemory());
      const len = Math.min(text.length, Math.max(0, (maxLen | 0) - 1));
    ...
    // Caption active bit for DefWindowProc WM_NCPAINT. Default: frontmost
    // visible top-level by zOrder. A host may override this to also gate on
    // host-app focus (another host window active → all captions inactive).
    is_window_active: (hwnd) => {
      const r = ctx.renderer;
      if (!r || !r.windows) return 1;
      let top = hwnd >>> 0;
      const self = r.windows[top];
      if (self && self.isChild && self.parentHwnd) {
        // Walk to top-level via parent chain (cheap; depth is tiny).
        let cur = self;
        let guard = 0;
        while (cur && cur.isChild && cur.parentHwnd && guard++ < 32) {
          top = cur.parentHwnd >>> 0;
          cur = r.windows[top];
        }
      }
      let best = 0, bestZ = -1;
      for (const k of Object.keys(r.windows)) {
        const w = r.windows[k];
        if (!w || w.isChild || w.visible === false || w._minimized) continue;
        const z = w.zOrder || 0;
        if (z > bestZ) { bestZ = z; best = w.hwnd >>> 0; }
      }
      if (!best) return 1;
      return best === top ? 1 : 0;
    },
    ...
    menu_is: (hMenu) => {
      const h = hMenu >>> 0;
      if (!h) return 0;
      // GetMenu's sentinel handle (see $handle_GetMenu)
      if (h === 0x80001) return 1;
      // GetSystemMenu's sentinel (see $handle_GetSystemMenu)
      if (h === 0x40003) return 1;
      // GetSubMenu encodes as (hMenu & 0xFFFF) | ((pos+1) << 16). Accept
      // non-zero high-word handles whose low word is the GetMenu sentinel
      // low-word (1) or any live CreateMenu handle's low 16 bits.
      if ((h >>> 16) !== 0) {
        const lo = h & 0xFFFF;
        if (lo === 0x0001) return 1;
        if (ctx._hostMenus) {
          for (const k of ctx._hostMenus.keys()) {
            if ((k & 0xFFFF) === lo) return 1;
          }
        }
      }
      return ctx._hostMenus && ctx._hostMenus.has(h) ? 1 : 0;
    },
    menu_get_string: (hMenu, uIDItem, flags, bufWA, cchMax) => {
      const h = hMenu >>> 0;
      const items = ctx._hostMenus && ctx._hostMenus.get(h);
      if (!items) return 0;
      const byPos = ((flags >>> 0) & 0x400) !== 0; // MF_BYPOSITION
      let item = null;
      if (byPos) {
        item = items[uIDItem >>> 0] || null;
      } else {
        item = items.find((it) => (it.id >>> 0) === (uIDItem >>> 0)) || null;
      }
      const text = item && item.text ? String(item.text) : '';
      if (!bufWA || cchMax <= 0) return text.length;
      const bytes = new Uint8Array(ctx.getMemory());
      const len = Math.min(text.length, Math.max(0, (cchMax | 0) - 1));
      for (let i = 0; i < len; i++) bytes[bufWA + i] = text.charCodeAt(i) & 0xFF;
      bytes[bufWA + len] = 0;
      return len;
    },
    ...
    // ---- Mapping mode (SetMapMode / LPtoDP / DPtoLP / extents) ----
    // Device = (logical - windowOrg) * viewportExt/windowExt + viewportOrg.
    // Fixed-scale modes imply their extents at LOGPIXELS dpi (y inverted);
    // SetWindowExtEx/SetViewportExtEx only take effect in MM_ISOTROPIC/
    // MM_ANISOTROPIC, matching real GDI.
    gdi_set_map_mode: (hdc, mode) => {
      const dc = _getDcState(hdc);
      const prev = dc.mapMode || 1;
      dc.mapMode = mode | 0;
      return prev;
    },
    gdi_get_map_mode: (hdc) => _getDcState(hdc).mapMode || 1,
    gdi_set_window_ext: (hdc, x, y, waPrev) => {
      const dc = _getDcState(hdc);
      if (waPrev) {
        const dv = new DataView(ctx.getMemory());
        dv.setInt32(waPrev, dc.appWinExtX || 1, true);
        dv.setInt32(waPrev + 4, dc.appWinExtY || 1, true);
      }
      dc.appWinExtX = x | 0;
      dc.appWinExtY = y | 0;
      return 1;
    },
    gdi_set_viewport_ext: (hdc, x, y, waPrev) => {
      const dc = _getDcState(hdc);
      if (waPrev) {
        const dv = new DataView(ctx.getMemory());
        dv.setInt32(waPrev, dc.appVpExtX || 1, true);
        dv.setInt32(waPrev + 4, dc.appVpExtY || 1, true);
      }
      dc.appVpExtX = x | 0;
      dc.appVpExtY = y | 0;
      return 1;
    },
    gdi_get_window_ext: (hdc, waSize) => {
      const e = _mapExtents(_getDcState(hdc));
      if (waSize) {
        const dv = new DataView(ctx.getMemory());
        dv.setInt32(waSize, e.wx, true);
        dv.setInt32(waSize + 4, e.wy, true);
      }
      return 1;
    },
    gdi_get_viewport_ext: (hdc, waSize) => {
      const e = _mapExtents(_getDcState(hdc));
      if (waSize) {
        const dv = new DataView(ctx.getMemory());
        dv.setInt32(waSize, e.vx, true);
        dv.setInt32(waSize + 4, e.vy, true);
      }
      return 1;
    },
    // Transform a POINT array in place. toDevice=1 → LPtoDP, 0 → DPtoLP.
    gdi_map_points: (hdc, waPts, count, toDevice) => {
      if (!waPts || count <= 0) return 1;
      const dc = _getDcState(hdc);
      const e = _mapExtents(dc);
      const wox = dc.winOrgX | 0, woy = dc.winOrgY | 0;
      const vox = dc.vpOrgX | 0, voy = dc.vpOrgY | 0;
      const dv = new DataView(ctx.getMemory());
      for (let i = 0; i < count; i++) {
        const px = dv.getInt32(waPts + i * 8, true);
        const py = dv.getInt32(waPts + i * 8 + 4, true);
        let nx, ny;
        if (toDevice) {
          nx = Math.round(((px - wox) * e.vx) / e.wx) + vox;
          ny = Math.round(((py - woy) * e.vy) / e.wy) + voy;
        } else {
          nx = Math.round(((px - vox) * e.wx) / e.vx) + wox;
          ny = Math.round(((py - voy) * e.wy) / e.vy) + woy;
        }
        dv.setInt32(waPts + i * 8, nx | 0, true);
        dv.setInt32(waPts + i * 8 + 4, ny | 0, true);
      }
      return 1;
    },
