...
(function () {
  ...
  const TouchCursor = {
    ...
    // Some games paint their pointer into the framebuffer while continuing
    // to consume absolute mouse coordinates. Their app profile is stronger
    // than transient ShowCursor state: suppress the phone's DOM cursor for
    // the whole run, without depending on a first touch or fullscreen setup.
    _profileHidden() {
      const apps = typeof window !== 'undefined' && window.wineShell &&
    ...
    tick() {
      ...
      // Relative aiming has no screen pointer, even while a finger is down.
      const layout = window.TouchControls && window.TouchControls.layout;
      if (!booting && layout && layout.cursor === false) visible = false;
      // browser-input applies this class for Win32 ShowCursor/SetCursor(NULL)
      // and for absolute-input games that paint a cursor into their own
      // framebuffer. It hides the browser's hardware cursor via CSS; the
      // touch cursor is a DOM sprite, so it must honor the same state itself.
      // Keep the launch hourglass authoritative while no guest exists yet.
      const guestHidden = this.canvas && this.canvas.classList &&
        this.canvas.classList.contains('guest-cursor-hidden');
      if (!booting && (guestHidden || this._profileHidden())) visible = false;
