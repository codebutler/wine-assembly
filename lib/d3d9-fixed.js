...
(function(root,factory){
  ...
  function compile(draw,viewport,linkedPixel){
    ...
    if(fixedVS&&f.lighting)fail('lighting is not implemented');
    ...
    const specular=fixedVS&&(f.specular||f.fog||!fixedPS)?(input(10,1,false)||'vec4(0.0)'):'vec4(0.0)';
    ...
    pbody.push('gl_FragColor = current;');
      ...
      for(const name of names){
        let value;
        if(name==='d3d_color0')value=`clamp(${diffuse},0.0,1.0)`;
