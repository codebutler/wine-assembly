...
(function(root,factory){
  ...
  function compile(draw,viewport,linkedPixel){
    ...
    if(lighting){
      ...
      if(!material||!Array.isArray(lights)||lights.length>8)fail('invalid directional lighting state');
      ...
      lights.forEach((light,i)=>{
        if(light.type!==3)fail('point/spot lighting is not implemented');
        ...
        vbody.push(`vec3 ff_lightVector${i} = -${normalized(`(d3d_ff_view*${direction}).xyz`)};`,
          `ff_lightDiffuse += ${ld}.rgb*max(dot(ff_litNormal,ff_lightVector${i}),0.0);`,
          `ff_lightAmbient += ${la}.rgb;`);
      });
