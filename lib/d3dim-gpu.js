  ...
  class D3DIMGpu {
    constructor(options) {
      ...
      this.stats = {
        ...
        fences: 0, syncs: 0, uploads: 0, textureUploads: 0,
    ...
    // the op about to run overwrites every pixel anyway.
    _prepare(t, d, overwritesAll) {
      if (!t.check && t.dib === d.dib) return;
      const bytes = this.u8.subarray(d.dib, d.dib + d.pitch * d.height);
      if (!overwritesAll && (t.dib !== d.dib || !t.shadow || !bytesEqual(bytes, t.shadow))) {
        const start = now();
        t.device.updateColor(null, this._toBGRA(t, bytes, d.pitch), t.width * 4);
        this.stats.uploads++;
    ...
    _toBGRA(t, bytes, pitch) {
      const { width, height } = t, out = new Uint8Array(width * height * 4);
      if (t.bpp === 32) {
        for (let y = 0; y < height; y++) out.set(bytes.subarray(y * pitch, y * pitch + width * 4), y * width * 4);
        return out;
      }
      const rgb565 = t.format === 1;
      for (let y = 0; y < height; y++) {
        let s = y * pitch, o = y * width * 4;
        for (let x = 0; x < width; x++, s += 2, o += 4) {
          const p = bytes[s] | (bytes[s + 1] << 8);
          let r, g, b;
          if (rgb565) { r = p >> 11; g = (p >> 5) & 63; b = p & 31; g = (g << 2) | (g >> 4); }
          else { r = (p >> 10) & 31; g = (p >> 5) & 31; b = p & 31; g = (g << 3) | (g >> 2); }
          out[o] = (b << 3) | (b >> 2); out[o + 1] = g; out[o + 2] = (r << 3) | (r >> 2); out[o + 3] = 255;
        }
      }
      return out;
    }
    ...
    fence() {
      ...
      for (const t of this.targets.values()) {
        if (!t.dirty) continue;
        const start = now();
        const { pixels } = t.device.readColor(null);
        const dst = this.u8, pitch = t.pitch, width = t.width;
        if (t.bpp === 32) {
          for (let y = 0; y < t.height; y++) dst.set(pixels.subarray(y * width * 4, (y + 1) * width * 4), t.dib + y * pitch);
        } else {
          const rgb565 = t.format === 1, alphaBit = t.format === 3 ? 0x8000 : 0;
          for (let y = 0; y < t.height; y++) {
            let s = y * width * 4, o = t.dib + y * pitch;
            for (let x = 0; x < width; x++, s += 4, o += 2) {
              const b = pixels[s], g = pixels[s + 1], r = pixels[s + 2];
              const p = rgb565
                ? ((r >> 3) << 11) | ((g >> 2) << 5) | (b >> 3)
                : alphaBit | ((r >> 3) << 10) | ((g >> 3) << 5) | (b >> 3);
    ...
    _draw(wa) {
      ...
      const draw = {
        ...
        fixedFunction: {
          lighting: false, fog: false, specular: false, alphaTest: false, textureFactor: 0xffffffff,
