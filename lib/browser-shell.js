...
(function () {
  ...
  // command line by now, so joining only puts it in the room -- the rest is
  // the game's own menus, which lan.join.hint names.
  function startLanOffers(wine, app, sel, log) {
    const seen = new Set();
    let looking = false;
    ...
    wine._lanOffers = setInterval(async () => {
      ...
      try {
        ...
        const hint = app.lan.join && app.lan.join.hint;
        if (room.role === 'member' && hint) {
          showLanChip(`● LAN · in ${room.owner.name}'s room — ${hint}`, 15000);
        }
      } catch (e) {
  ...
  function createBrowserShell(deps) {
    ...
    async function launchAppInner(appKey, launchOpts) {
      ...
      if (autoRoom) {
        let owner = null;
        try {
          owner = firstHosted(await withTimeout(
            VlanRoom.hostedRooms({ join: { exe: app.lan.exe || sel } }), 2500));
        } catch (_) { /* signed out, offline, or slow: launch as usual */ }
        if (owner && await askToJoin(owner, app) === 'join') {
          try {
        ...
        askWine.openLanLink = () => {
          ...
          if (autoRoom) {
            (async () => {
              ...
              let signedOut = false;
              try {
                owner = firstHosted(await VlanRoom.hostedRooms({ join: { exe: app.lan.exe || sel } }));
              } catch (e) { signedOut = !!(e && e.needsLogin); }
              if (owner && await askToJoin(owner, app, { skipLabel: 'Play offline' }) !== 'join') {
      ...
      wine.onStopped = stoppedWine => {
        ...
        if (stoppedWine._lanRoom) { stoppedWine._lanRoom.close(); stoppedWine._lanRoom = null; }
      ...
      if (ok) {
        ...
        syncTouchControls();
        if (autoRoom && !lanRoom) startLanOffers(wine, app, sel, log);
