...
(function () {
  ...
  async function openAutoRoom(app, sel, log, preferOwner) {
  ...
  // ---- invite links ------------------------------------------------------
  //
  // ?app=ID&room=USERID: open this game and join that person's room. Only for
  // the app the link names, so the same page launching something else later
  ...
  function lanInviteUrl(sel, ownerUserId) {
  ...
  // One small button under the status chip while this page is in a room. A
  // phone gets the share sheet; anything else copies the link, and says so.
  function showLanInvite(sel, room) {
    if (typeof document === 'undefined' || !document.body || !room.ownerUserId) return;
    hideLanInvite();
    const link = lanInviteUrl(sel, room.ownerUserId);
    const button = document.createElement('button');
    button.id = 'wine-lan-invite';
    button.type = 'button';
    button.textContent = 'Invite';
    button.dataset.link = link;
    Object.assign(button.style, {
      position: 'fixed', left: 'calc(8px + env(safe-area-inset-left, 0px))',
      top: 'calc(32px + env(safe-area-inset-top, 0px))', zIndex: '2147483000',
      padding: '2px 10px', color: '#fff', background: 'rgba(0,0,0,.62)',
      border: '1px solid rgba(255,255,255,.35)', borderRadius: '10px',
      font: '12px Arial, sans-serif', cursor: 'pointer',
    });
    button.addEventListener('click', async e => {
      e.stopPropagation();
      if (navigator.share && /Mobi|iPhone|iPad|Android/.test(navigator.userAgent)) {
        try { await navigator.share({ title: 'Join my game', url: link }); return; } catch (_) {}
      }
      try {
        await navigator.clipboard.writeText(link);
        showLanChip('LAN · invite link copied', 4000);
      } catch (_) {
        window.prompt('Send this link to a friend:', link);
      }
    });
    lanNoticeHost().appendChild(button);
  }

  function hideLanInvite() {
    const old = typeof document !== 'undefined' && document.getElementById('wine-lan-invite');
    if (old) old.remove();
  }
  ...
  function createBrowserShell(deps) {
    ...
    async function launchAppInner(appKey, launchOpts) {
      ...
      if (autoRoom) {
        ...
        let invited = null;
        try {
          const rooms = await withTimeout(VlanRoom.hostedRooms(
            { join: { exe: app.lan.exe || sel }, includeIdle: !!invite }), 2500);
          invited = invite ? rooms.find(r => r.userId === invite) || null : null;
          owner = invited || firstHosted(rooms.filter(r => r.hosting));
        } catch (_) { /* signed out, offline, or slow: launch as usual */ }
        if (invite && !invited) log.textContent += 'LAN: whoever sent the invite is not in a room right now.\n';
        ...
        } else if (owner && (owner === invited || await askToJoin(owner, app) === 'join')) {
          try {
        ...
        askWine.openLanLink = () => {
          ...
          if (autoRoom) {
            (async () => {
              ...
              let signedOut = false;
              try {
                const rooms = await VlanRoom.hostedRooms(
                  { join: { exe: app.lan.exe || sel }, includeIdle: !!invite });
                owner = (invite && rooms.find(r => r.userId === invite))
                  || firstHosted(rooms.filter(r => r.hosting));
              } catch (e) { signedOut = !!(e && e.needsLogin); }
              const invitedHere = !!owner && owner.userId === invite;
              if (owner && !invitedHere
                  && await askToJoin(owner, app, { skipLabel: 'Play offline' }) !== 'join') {
                ...
                try {
                  const room = await openAutoRoom(app, sel, log, owner && owner.userId);
