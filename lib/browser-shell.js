  ...
  function createBrowserShell(deps) {
    ...
    function syncTouchControls() {
      ...
      if (sharedRenderer) {
        ...
        // for SM_CXSCREEN.
        sharedRenderer.singleAppZoom =
          (last && typeof last.mobileZoom === 'number' && last.mobileZoom > 1)
            ? last.mobileZoom : 0;
      }
    ...
    async function launchAppInner(appKey) {
      ...
      const app = apps[sel];
      if (!app) return;
      const launchArgs = SINGLE_APP() && app.singleAppArgs !== undefined
      ...
      if (ok) {
        ...
        runningApps.push({ wine, name: sel, appIndex, runSliceAppKey,
          ...
          mobileZoom: (typeof app.mobileZoom === 'number' && app.mobileZoom > 1)
            ? app.mobileZoom : 0,
