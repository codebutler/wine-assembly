...
/* The view-mode toggle and the keyboard pill. They ride in the bottom-right
   stack UNDER the app's own buttons, not pinned to the left edge: an edge chip
   floats over the picture, is clipped by the safe-area inset on a notched
   phone, and reads as debris rather than a control. Down here they are in the
   band presentation already keeps clear, so they cover nothing. */
...
`;
  ...
  const TouchControls = {
    ...
    _addButton(spec) {
      ...
      else el.textContent = spec.label === undefined ? String(spec.vk) : String(spec.label);
      ...
      if (board) {
        ...
        const size = Math.max(44, spec.size | 0 || 52);
    ...
    // Both chips share one row, created before any app button so the flex
    ...
    syncViewMode() {
      ...
      // ...and it is not offered in landscape for a crop that declares one
      // framing there. See _enforceLandscapeView: with one framing there are
      // not two states to toggle between, and a control that cannot change
      // anything is worse than no control.
      this._modeEl.hidden = !crop || this._landscapeSingleView();
    ...
    // Is this layout in the orientation where it has said there is only one
    // sensible framing? Pinball in landscape is the case: a tall table fitted
    // to the short edge (which landscape is short of in HEIGHT) fills the
    // screen top to bottom, and the alternative -- the whole 641x481 scene,
    // score panel included -- is both smaller and letterboxed.
    _landscapeSingleView() {
      if (!this.layout || this.layout.landscapeView !== 'crop') return false;
      const host = this.el && this.el.getBoundingClientRect
        ? this.el.getBoundingClientRect() : null;
      if (!host || !(host.width > 0) || !(host.height > 0)) return false;
      return host.width > host.height;
    },

    // Landscape has one framing, so put the renderer in it. A pinch is left
    // alone while it is happening -- snapping back mid-gesture would fight the
    // finger -- and it settles here on the next poll, which is the same thing
    // the missing chip says: this orientation does not offer the other view.
    _enforceLandscapeView() {
      const renderer = this._renderer_();
      if (!renderer || !this._landscapeSingleView()) return;
      if (Number.isFinite(renderer._pinchProgress)) return;
      if (renderer.viewMode === 'zoom') return;
      if (this._enforcingView) return;
      this._enforcingView = true;
      try { this.setViewMode('zoom'); } finally { this._enforcingView = false; }
    },
    ...
    layoutZones() {
      ...
      for (const corner of ['bl', 'br']) {
        if (this._corners && this._corners[corner]) {
          ...
          this._corners[corner].style.paddingLeft = mouseLandscape
            ? (coversNotch ? 'calc(8px + env(safe-area-inset-left, 0px))' : '8px') : '';
    ...
    //
    // The crop's fractions are of the WINDOW rect and the viewport's source is
    // in canvas coordinates, so this needs the window's place on the canvas.
    // It is only used for an exclusive-fullscreen presentation -- the one this
    // overlay's zones exist for -- where the window IS the canvas, so the two
    // coincide. Returns null for anything else, and every caller falls back to
    // the old fractions-of-the-presented-rect placement.
    _cropRectClient(app) {
      const r = this._renderer_();
      ...
      const cw = r.canvas && r.canvas.width, ch = r.canvas && r.canvas.height;
      if (!(cw > 0) || !(ch > 0)) return null;
      const kx = app.w / v.cropW, ky = app.h / v.cropH;
      return {
        x: app.x + (crop.x * cw - v.cropX) * kx,
        y: app.y + (crop.y * ch - v.cropY) * ky,
        w: crop.w * cw * kx,
        h: crop.h * ch * ky,
      };
    },
    ...
    _layoutBoardButtons(app, host) {
      ...
      for (const el of list) {
        ...
        const size = el._tcBoardSize || 52;
        const left = corner === 'tl' || corner === 'bl';
        const top = corner === 'tl' || corner === 'tr';
        const x = left ? app.x + INSET : app.x + app.w - INSET - size;
        const y = top ? app.y + INSET : app.y + app.h - INSET - size;
    ...
    // Captions go in the letterbox, never on the artwork. Two cases, and they
    // are genuinely different shapes rather than one rule with a special case:
    //
    //   PORTRAIT -- there is a band under the picture (the controls' own clear
    //     area, plus whatever the fit left over). Each caption sits just below
    //     the picture at its zone's horizontal centre. "Right under the table"
    //     is what was asked for and it is also the only reading that works:
    //     the caption is directly under the thing it names.
    //   LANDSCAPE -- a tall table fitted to the height leaves NO band under it
    //     at all, only gutters at the sides. So each caption goes to the
    //     gutter on its own side, stacked up from the bottom. Not as direct,
    //     and it is still the only place that covers no playfield.
    // `app` is the TABLE (the crop), `picture` the whole presented rect. They
    // differ in Normal view, where the presented rect also carries the score
    // panel: the captions anchor to the table, but the gutter they fall back
    // to in landscape has to clear the picture, or one of them lands on the
    // score readout.
    _layoutCaptions(app, host, usableHost, picture) {
      const list = this._captions || [];
      if (!list.length) return;
      if (!app || !(app.w > 0) || !(app.h > 0)) {
        for (const cap of list) cap.hidden = true;
        return;
      }
      const GAP = 8, LINE = 16;
      const below = usableHost.bottom - (app.y + app.h);
      if (below >= GAP + LINE) {
      ...
      // Landscape: side gutters. The flippers are at the FOOT of the table,
      // so the captions stay low in the gutter -- but above whatever the
      // corner rail on that side is already holding, measured rather than
      // assumed, because that stack's height depends on the app's own buttons.
      const frame = picture && picture.w > 0 ? picture : app;
      const leftGutter = frame.x - usableHost.left;
      const rightGutter = usableHost.right - (frame.x + frame.w);
      const floorOf = (corner) => {
        const node = this._corners && this._corners[corner];
        const r = node && node.getBoundingClientRect ? node.getBoundingClientRect() : null;
        const top = r && r.height > 0 ? r.top : usableHost.bottom - 18;
        return Math.min(top, usableHost.bottom - 18);
      };
      const floors = { bl: floorOf('bl'), br: floorOf('br') };
      let leftRow = 0, rightRow = 0;
      for (const cap of list) {
        const onLeft = cap._tcAt < 0.5;
        const gutter = onLeft ? leftGutter : rightGutter;
        if (gutter < 48) { cap.hidden = true; continue; }
        cap.hidden = false;
        cap.style.transform = 'translateX(-50%)';
        const centre = onLeft
          ? usableHost.left + leftGutter / 2
          : frame.x + frame.w + rightGutter / 2;
        const row = onLeft ? leftRow++ : rightRow++;
        const floor = floors[onLeft ? 'bl' : 'br'];
        cap.style.left = (centre - host.left) + 'px';
        cap.style.top = Math.max(usableHost.top + 8 - host.top,
          floor - host.top - GAP - LINE - row * (LINE + GAP)) + 'px';
      }
    },
    ...
    // Where the two utility pills go.
    //
    // They used to hunt for dead space: the letterbox bar, else the gap
    // between the two corner clusters, else the left edge at 35% opacity.
    // That put them somewhere different in every app, and on a layout with a
    // cross pad in one corner and a command button in the other the last
    // branch is what won -- a half-visible "Fill" chip pinned to the left
    // bezel, clipped by the notch inset, floating over the board. They ride
    // the bottom-right stack instead now, below the app's own buttons, in
    // space presentation already keeps clear of the picture (see
    // Win98Renderer.singleAppStageShare). That placement is the flex row they
    // are appended to, so there is nothing to compute here.
    //
    // A board layout is the exception: its corners are positioned against the
    // presented picture rather than the phone, so its pills are placed
    // explicitly against the action button.
    _placeModeToggle(app, host) {
      // Both pills are placed together -- two independent placers would each
      // centre themselves in the same gap and land on top of each other.
      const pills = [this._keyEl, this._modalControlsHidden ? null : this._modeEl]
        .filter(Boolean);
      if (!pills.length) return;
      // ...but a hidden one takes no SLOT. syncViewMode hides the fit/fill
      // chip for an app with no crop, and in the orientation where the crop
      // offers only one framing (Pinball in landscape); reserving 50px beside
      // the keyboard pill for a chip that is display:none leaves a hole.
      // It is still reparented and normalized with the rest, so unhiding it
      // needs no second code path.
      const shown = pills.filter(p => !p.hidden);
      const size = 40;
      const PILL_GAP = 10;
      const spanFor = (n) => n * size + (n - 1) * PILL_GAP;
      if (!this._modalControlsHidden && this.layout && this.layout.boardLayout) {
        for (const p of pills) {
          // Out of the chip row and back onto the overlay itself. The
          // coordinates below are relative to the host, and an absolutely
          // positioned element resolves them against its nearest positioned
          // ancestor -- which, while it sits in the row, is the corner stack.
          // Rodent's Revenge put its pills 1236px down a 740px screen.
          if (p.parentNode !== this.el) this.el.appendChild(p);
          p.style.position = 'absolute';
          p.style.transform = 'none';
          p.style.width = size + 'px';
        }
        // The app's action button, not one of the pills -- they share the
        // corner now and are added first, so an unfiltered find returns a pill
        // and places the pair against itself.
        const action = this._widgets.find(w => w._tcCorner === 'br' &&
          w.className !== 'tc-key' && w.className !== 'tc-mode' &&
          w.getBoundingClientRect);
        const r = action && action.getBoundingClientRect();
        if (r && r.width > 0) {
          const span = spanFor(Math.max(1, shown.length));
          let x = r.right - host.left - span;
          for (const p of pills.slice().reverse()) {
            p.style.left = x + 'px';
            p.style.top = (this.layout.mouseJoystick
              ? r.bottom - host.top + 18
              : Math.max(4, r.top - host.top - size - 12)) + 'px';
            p.style.opacity = '1';
            // Only a visible pill advances the cursor. A hidden one is given
            // the coordinates anyway -- it is display:none, so it covers
            // nothing, and unhiding it never finds it unpositioned.
            if (!p.hidden) x += size + PILL_GAP;
          }
          return;
        }
      }
      // Everything else: back to the flex row they belong to. Clearing the
      // inline styles matters -- a layout swap from a board profile to an
      // ordinary one would otherwise leave them absolutely positioned at
      // whatever the board's action button implied.
      const row = this._chipRow();
      for (const p of [this._modeEl, this._keyEl]) {
        if (!p || !pills.includes(p)) continue;
        // Board layouts lift these onto the overlay; put them back in the row
        // in their created order, or a swap away from a board profile leaves
        // them absolutely positioned where its action button used to be.
