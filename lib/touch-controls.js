  ...
  const TouchControls = {
    ...
    _centerCornersInGutters(app, host, usableHost) {
      ...
      for (const corner of ['bl', 'br']) {
        ...
        if (!el) continue;
        let dx = 0;
        if (active && !(this.layout && this.layout.screenAnchored) && el.getBoundingClientRect) {
    ...
    // bezel, clipped by the notch inset, floating over the board. They
    // normally ride the bottom-right stack below the app's own buttons, in
    // space presentation already keeps clear of the picture (see
    // Win98Renderer.singleAppStageShare). An app may place its keyboard chip
    // in another corner beside its own button.
    //
    ...
    _placeModeToggle(app, host) {
      ...
      for (const p of [this._modeEl, this._keyEl]) {
        ...
        const row = p === this._keyEl && this.layout && this.layout.keyboardCorner
          ? this._rowNode(normalizeCorner(this.layout.keyboardCorner),
            Number.isInteger(this.layout.keyboardRow) ? this.layout.keyboardRow : -1)
          : this._chipRow();
