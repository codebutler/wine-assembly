  ...
  const TouchControls = {
    ...
    _layoutCaptions(app, host, usableHost, picture) {
      ...
      for (const cap of list) {
        ...
        // Clear the placement with the caption. `hidden` alone leaves the
        // inline left/top from whichever branch ran last, so a node that is
        // hidden here still carries a portrait position -- which reads, in any
        // later inspection, as a caption that IS placed and merely invisible.
        if (gutter < 48) {
          cap.hidden = true;
          cap.style.left = '';
          cap.style.top = '';
