...
`;
  ...
  const TouchControls = {
    ...
    syncViewMode() {
      ...
      if (named) this._modeEl.textContent = named;
      else this._modeEl.innerHTML = this._viewIcon(!zoom);
    ...
    layoutZones() {
      ...
      const crop = renderer && renderer.mobileCrop;
      const zoneApp = this.layout && this.layout.zonesOnCrop && crop && renderer.viewMode !== 'zoom'
    ...
    _placeModeToggle(app, host) {
      ...
      const pills = [this._keyEl, this._modalControlsHidden ? null : this._modeEl].filter(Boolean);
