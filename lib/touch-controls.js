...
`;
  ...
  const TouchControls = {
    ...
    _addButton(spec) {
      if (!spec || (!Number.isFinite(spec.vk) && !Number.isFinite(spec.char) &&
      ...
      const hold = command === null && mouseButton === null && charCode === null &&
        spec.hold !== false;
      const vk = Number.isFinite(spec.vk) ? spec.vk | 0 : 0;
    ...
    // Which view this layout has said is the only sensible one in landscape,
    // or null when it offers both. Pinball is the case: 'window' means the
    // whole 641x481 scene, score panel included, fitted to the short edge --
    // which in landscape is the HEIGHT, so it runs screen top to bottom with
    // no black bars, and the gutters land left and right where there is
    // nothing to lose. Cropping to the table there would throw away the score
    // panel for no extra magnification worth having.
    // 'crop' is the other value the mechanism accepts, for a layout that does
    // want its mobileCrop forced in landscape.
    ...
    _layoutBoardButtons(app, host) {
      ...
      for (const el of list) {
        ...
        if (fit && fit.r > 0) {
          ...
          size = Math.max(44, Math.min(fitted, el._tcBoardSize || 52));
