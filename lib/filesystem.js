...
class VirtualFS {
  ...
  findFirstFile(pattern) {
    ...
    // try matching just the filename anywhere in the VFS. As with createFile,
    // never apply this media fallback to C: paths: an absent local override
    // must not appear to exist merely because the cache has the same basename.
    if (results.length === 0 && !pat.includes('*') && !pat.includes('?') &&
        !norm.startsWith('c:') && !this.dirs.has(dir)) {
      for (const [path, entry] of this.files) {
