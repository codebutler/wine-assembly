...
(function (root, factory) {
  ...
  // Process-wide load-immune counters, read by tools/page-probes/read-gl-stats.js.
  // They answer "how many guest GL calls cross into JS per frame, and how many
  // glBegin/glEnd spans do they make" -- the numbers that size moving this
  // encoder into WAT. Plain increments; nothing reads them on the hot path.
  const stats = {
    calls: 0, byOpcode: new Float64Array(128), spans: 0,
    packedRecords: 0, packedVertices: 0, flushes: 0,
  };
  ...
  class Encoder {
    ...
    _capturePacked(mode, vertices) {
      ...
      stats.packedRecords++;
      stats.packedVertices += vertices.length / VERTEX_FLOATS;
    ...
    call(opcode, stackWa, aux) {
      ...
      stats.calls++;
      if (opcode >= 0 && opcode < 128) stats.byOpcode[opcode]++;
      const memory = this.getMemory();
  ...
  return {
    ...
    PACKED_DRAW_OPCODE, WAT_STREAM_FLUSH_OPCODE, VERTEX_FLOATS, ARG_WORDS, BARRIERS,
    Encoder, pointerSpec, normalizeImmediate, memoryBatch, replay, stats,
  };
});
