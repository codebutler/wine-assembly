...
// lib/audio-voice-worklet.js). Default OFF; every routing gate, including the
// one that keeps one-shot Play() away from it, lives in that file.
      ...
      playRing(id, ptr, len, startOff, loop) {
        ...
        if (loop === 2) {
          ...
          if (_worklet && v.workletNode &&
              _worklet.route(ctx, this._ac, v, ctx.getMemory(), ptr, len, 0, 1)) {
          ...
          if (ac && live && old.loop) {
            ...
            const elapsed = Math.max(0, when - v.playStart) * rateScale;
        ...
        _markAudioMixerPeak(1,
          _measurePcmPeak(pcm, ptr + playFrom, playLength, v.channels, v.bits),
          loop ? 500 : durationMs);
