...
//
// Returns 'focus', 'refocus', 'blur', or 'none' (leave the DOM alone).
function keyboardProxyAction(state) {
  const s = state || {};
  const hasCaret = !!s.hasCaret;
  const proxyFocused = !!s.proxyFocused;
  // DOM focus granted after the guest's asynchronous dialog initialization
  // does not imply that Safari opened its software keyboard. Retry once from
  // a trusted tap, but never interrupt a keyboard that is already visible.
  if ((hasCaret || s.manual) && proxyFocused && s.gesture &&
      s.focusNeedsGesture && !s.keyboardVisible) return 'refocus';
