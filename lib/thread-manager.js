...
class ThreadManager {
    ...
    for (let slice = 0; slice < numSlices; slice++) {
      ...
      for (const [handle, thread] of this._threadEntries(threadOrderOptions)) {
        ...
        if (yieldReason === 7) {
          ...
          thread.waitPolls = 0;
          // resume_message_wait owns EIP: the return address, or a hook
          // (WH_KEYBOARD) that GetMessage entered on the way out.
          if (!(e.resume_message_wait && (e.resume_message_wait() | 0))) continue;
  ...
  checkMainYield() {
    ...
    if (yr === 7) {
      ...
      // resume_message_wait sets EIP itself from the return address at
      // [esp] -- never _readWaitReturnAddress(), whose EXE-only code bounds
      // misjudge a pump living in a DLL (mfc42.dll) -- or leaves it on a
      // WH_KEYBOARD hook GetMessage entered on the way out.
      if (!(e.resume_message_wait && (e.resume_message_wait() | 0))) return true;
      return false;
    }
    if (yr === 9) {
