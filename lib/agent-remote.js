...
// The renderer that owns the guest's input: the page-wide one when the shell
// shares it, else the newest running app's.
function liveRenderer() {
  return window.sharedRenderer
...
function execEntry(entry) {
  ...
  switch (kind) {
    ...
    // DirectInput games (Black & White 2) read relative motion and button
    // records, not the window's cursor, so a synthetic MouseEvent -- which
    // carries no movementX/Y and never holds pointer lock -- clicks nothing.
    // These are the browser twins of run.js's relmousemove / di-mousedown /
    // di-mouseup --input actions and go through the same renderer entry
    // points the pointer-lock handlers in browser-input.js use.
    case 'relmousemove': {
      const r = liveRenderer();
      if (!r || !r.handleRelativeMouseMove) throw new Error('no renderer with relative mouse support');
      r.handleRelativeMouseMove(x, y);
      return { did: entry };
    }
    case 'di-mousedown': case 'di-mouseup': {
      const r = liveRenderer();
      if (!r) throw new Error('no renderer');
      const button = parts[1] === undefined || parts[1] === '' ? 1 : parseInt(parts[1], 10);
      if (button !== 1 && button !== 2) throw new Error(`${kind} button must be 1 or 2`);
      const mask = button === 2 ? 0x0002 : 0x0001;
      const down = kind === 'di-mousedown';
      const wasDown = !!((r._mouseButtonsMask || 0) & mask);
      r._mouseButtonsMask = down ? ((r._mouseButtonsMask || 0) | mask) : ((r._mouseButtonsMask || 0) & ~mask);
      if (!r._asyncPressedKeys) r._asyncPressedKeys = Object.create(null);
      if (down) r._asyncPressedKeys[button === 2 ? 0x02 : 0x01] = true;
      if ((!down || wasDown !== down) && r._queueDirectInputMouseButton) {
        r._queueDirectInputMouseButton(r._pointerInputMemory || r.wasmMemory, mask, down);
      }
      if (r._signalDirectInputDevice) r._signalDirectInputDevice(2);
      return { did: entry };
    }
    default:
      throw new Error(`the browser session supports click/dblclick/rclick/mouse*/wheel/key*/relmousemove/di-mouse* entries, not ${JSON.stringify(kind)}`);
  }
}
