...
function applyExeCompatibilityPatches(exeName, exports, memoryBuffer, opts) {
  const { skip = false, disableCopy32Counted = false } = opts || {};
  if (skip) return 0;
  // The phone A/B on Diablo Shareware's Tristram renderer measured 7.6 vs
  // 5.0 guest fps on the same WASM build. This gate lives in shared memory,
  // so setting it through the host-shadow instance also enables Worker decode.
  // Keep every other executable on the explicit ?copy32-counted test path.
  if (String(exeName || '').toLowerCase() === 'diablo_s.exe' &&
      !disableCopy32Counted && exports &&
      typeof exports.set_loop_copy32_counted_emit === 'function') {
    exports.set_loop_copy32_counted_emit(1);
  }
