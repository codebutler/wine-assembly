...
(function (root, factory) {
  ...
  class Bridge {
    ...
    _submit(entry, opcode, payload) {
      ...
      if (entry.deferredError) { const error = entry.deferredError; entry.deferredError = null; throw error; }
    ...
    _issue(entry, opcode, payload, finalize) {
      ...
      const value = this._submit(entry, opcode, payload);
    ...
    call(opcode, address, aux) {
      ...
      try {
        ...
        if (!stride || stride>255) throw new Error('invalid D3D9 draw byte range');
        ...
        const indexPointer=u32(address+44), vertexPointer=u32(address+32), table=u32(address+60);
        const binding=index=>{
          if(!index)return {pointer:vertexPointer,stride};
          if(!table)throw new Error(`D3D9 stream ${index} has no binding in this draw`);
        ...
        return this._issue(entry, Stream.OPCODES.DRAW, { primitive, primitiveCount, stride:packedStride, fixedFunction,fogState,
