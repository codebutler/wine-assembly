...
(function (root, factory) {
  ...
  class Bridge {
    ...
    _rememberTexture(key, entry) {
      ...
      if (bytes > this.maxTextureSnapshotBytes) return;
      entry.bytes = bytes;
        ...
        for(let stage=0;stage<6;++stage){
          ...
          for(let level=lod;level<levelCount;++level){const m=t+64+(face*levelCount+level)*32,w=u32(m),h=u32(m+4);
            ...
            if(cached&&cached.seq===seq&&cached.bits===bits&&cached.byteLength===byteLength
              ...
              levels.push({width:w,height:h,pixels:cached.pixels});
              continue;
            }
            let pixels;
            ...
            this._rememberTexture(m,{seq,bits,byteLength,format,width:w,height:h,
