  ...
  class Bridge {
    ...
    _execute(entry, command) {
      ...
      try {
        ...
        else if (command.opcode === op.CLEAR) entry.device.clear(p.color, p.flags,p.depth,p.rects,p.depthAttachment,p.stencil);
        else if (command.opcode === op.DRAW) entry.device.draw(p);
        else if (command.opcode === op.PRESENT) {
          const gpu = entry.device.gpu, gl = gpu.gl, { width, height } = p;
    ...
    call(opcode, address, aux) {
      ...
      if(opcode===0x30008){
        try{
          const entry=this.devices.get(aux>>>0);if(!entry)return 1;
          const submitted=this._submit(entry,Stream.OPCODES.RESOURCE_RELEASE,{kind:'depth',id:u32(address+36)});
      ...
      try {
        ...
        if (!entry && opcode !== 0x30001 && opcode !== 0x30003 && opcode !== 0x3000b && opcode !== 0x3000c) return 0;
        if (!entry) {
        ...
        if (opcode === 0x30003) {
          ...
          if(count){
            ...
            for(let i=0;i<count;i++){
              ...
              const clipped=[Math.max(l,vx),Math.max(t,vy),Math.min(r,vx+vw),Math.min(b,vy+vh)];
              if(clipped[2]>clipped[0]&&clipped[3]>clipped[1])rects.push(clipped);
            }
          }else rects.push([vx,vy,vx+vw,vy+vh]);
          return this._result(entry, this._submit(entry, Stream.OPCODES.CLEAR, {
            color: [((color>>>16)&255)/255,((color>>>8)&255)/255,(color&255)/255,(color>>>24)/255], flags,depth:z,stencil:u32(address+24),rects,depthAttachment }));
        }
        ...
        const shader = offset => {
          ...
          if (ex?.d3d_shader_ir_compile) {
            ...
            return this.shaderCompiler.compile(ptr+24,length/4);
          }
        ...
        return this._result(entry, this._submit(entry, Stream.OPCODES.DRAW, { primitive, primitiveCount, stride, fixedFunction,fogState,
          ...
          vertices, indices, attributes, textures, bumpStates,depthAttachment,
