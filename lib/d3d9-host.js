  ...
  class Bridge {
    ...
    _execute(entry, command) {
      ...
      try {
        ...
        else if (command.opcode === op.RESOURCE_UPDATE && p.kind==='color') entry.device.updateColor(p.resource,p.pixels,p.pitch);
    ...
    call(opcode, address, aux) {
      ...
      try {
        ...
        if(colorPointer&&opcode!==0x30002){
          ...
          if(u32(p+12)!==0xd3d90006||u32(p+8)!==id||u32(p+56))throw new Error('invalid or locked D3D9 color attachment');
          colorAttachment={id:u32(p+36),width:u32(p+20),height:u32(p+24),format:u32(p+28)};
          width=colorAttachment.width;height=colorAttachment.height;
          if(!entry.colors)entry.colors=new Set();
          if(!entry.colors.has(colorAttachment.id)){
            const created=this._submit(entry,Stream.OPCODES.RESOURCE_CREATE,{kind:'color',resource:colorAttachment,
              pitch:u32(p+48),pixels:new Uint8Array(memory,g2w(u32(p+40)),u32(p+52)).slice()});
            if(created&&typeof created.then==='function')created.catch(error=>{this.lastError=String(error);});
            entry.colors.add(colorAttachment.id);
          }
        }
