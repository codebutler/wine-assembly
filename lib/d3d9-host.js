  ...
  class Bridge {
    ...
    call(opcode, address, aux) {
      ...
      try {
        ...
        if(opcode===0x30015){
          const resource=ensureColor(g2w(u32(address+24))),bits=u32(address+28),sourcePitch=u32(address+32);
          const rect={x:u32(address+36),y:u32(address+44),width:u32(address+48),height:u32(address+52)};
          if(!rect.width||!rect.height||rect.x>resource.width||rect.y>resource.height||
            rect.width>resource.width-rect.x||rect.height>resource.height-rect.y||u32(address+56)!==resource.format||
