...
(function(root,factory){
  ...
  'use strict';
  const DXT1=0x31545844,DXT5=0x35545844;
  const color565=c=>{
    const r=c>>>11,g=(c>>>5)&63,b=c&31;
    return [(r<<3)|(r>>>2),(g<<2)|(g>>>4),(b<<3)|(b>>>2),255];
  };
  function decode(bytes,width,height,format){
    if(![DXT1,DXT5].includes(format))throw new Error('unsupported DXT format');
    ...
    for(let by=0;by<rows;by++)for(let bx=0;bx<columns;bx++){
      const start=(by*columns+bx)*blockBytes,c=start+(format===DXT5?8:0);
