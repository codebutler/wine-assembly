...
| 7 | **PARTIAL** | `gen_dispatch.js` now emits 186 opt-in `test_call:true` wrappers from each API's `nargs`; the 63 helpers that synthesize guest stacks or carry other setup remain hand-written. The same table/generator owns the first 14 constant compatibility stubs (the BASS no-audio surface) as explicit `{pop,ret}` data, and the silent-handler ratchet distinguishes those reviewed metadata stubs from its 391 remaining hand-written quiet handlers. Migrating/reviewing that remaining inventory is still open. |
