...
**The compiled bytes are not the text in `src/`: dispatch is replicated first.** `tools/watx-closure.js` (and the browser's `lib/watx-launcher.js`) run `lib/dispatch-replicate.js` over the closure in memory, inlining `$next`'s body at every `(return_call $next)` so each handler ends in its own `return_call_indirect` — 419 sites in 384 functions, +32 KB of wasm, **−4.86% gameplay CPU** on StarCraft against a 1.13% null band (branch-miss rate unchanged at 0.24%; the win is the deleted call, frame setup and stack check per dispatch, not prediction). `src/` on disk keeps the one-site text, so grep, `combined.wat` and the census gates all see one `$next`. `WINE_DISPATCH=shared bash tools/build.sh` (or `--dispatch=shared`, `?dispatch=shared` in the browser) builds the one-site module for an A/B; the banner line `WATX dispatch:` says which one you got.
