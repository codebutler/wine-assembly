...
(func $d3d9_backbuffer_dc_sync (param $device i32) (param $upload i32) (result i32)
  (local $desc i32) (local $result i32) (local $state i32)
  ...
  (if (local.get $upload) (then
    ...
    (if (i32.and (load.field DxObject flags (call $d3ddev_rt_entry (local.get $device)))
      (i32.shl (i32.const 1) (i32.const 26))) (then
      (local.set $state (call $g2w (call $d3d9_program_state (local.get $device))))
      (i32.store offset=36 (local.get $desc) (i32.load offset=22028 (local.get $state)))
      (i32.store offset=44 (local.get $desc) (i32.load offset=22032 (local.get $state)))
      (i32.store offset=48 (local.get $desc) (i32.load offset=22036 (local.get $state)))
      (i32.store offset=52 (local.get $desc) (i32.load offset=22040 (local.get $state)))
      (i32.store offset=28 (local.get $desc) (i32.add (i32.load offset=28 (local.get $desc))
        (i32.add (i32.mul (i32.load offset=44 (local.get $desc)) (i32.load offset=32 (local.get $desc)))
          (i32.mul (i32.load offset=36 (local.get $desc)) (i32.const 4)))))))
...
(func $d3d9_backbuffer_lock (param $surface i32) (param $out i32) (param $rect i32) (param $flags i32)
  ...
  (local.set $state (call $g2w (call $d3d9_program_state (local.get $device))))
  (block $snapshot
  (br_if $snapshot (global.get $d3d_render_token))
  ...
  (if (local.get $rect) (then
    ...
    (local.set $width (i32.sub (i32.load offset=8 (local.get $rw)) (local.get $left)))
    (local.set $height (i32.sub (i32.load offset=12 (local.get $rw)) (local.get $top)))))
  (if (i32.eqz (global.get $d3d_render_token)) (then
    ...
    (i32.store offset=22024 (local.get $state) (local.get $dest))
    (i32.store offset=22028 (local.get $state) (local.get $left))
    (i32.store offset=22032 (local.get $state) (local.get $top))
    (i32.store offset=22036 (local.get $state) (local.get $width))
    (i32.store offset=22040 (local.get $state) (local.get $height))
  ...
  (local.set $dest (i32.load offset=22024 (local.get $state)))
  (local.set $left (i32.load offset=22028 (local.get $state)))
  (local.set $top (i32.load offset=22032 (local.get $state)))
