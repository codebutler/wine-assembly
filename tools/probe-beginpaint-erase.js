...
// Contract probe: CURRENTLY FAILS on main. See the candidate investigation in
// docs/review-win16-windowpos.md. This is not a passing regression-suite test.
...
(async () => {
  ...
  const record = e.guest_alloc(24), ps = e.guest_alloc(64);
  const brush = e.test_call_CreateSolidBrush(0xff);
  for (const handled of [0, 1]) for (const background of [0, brush]) for (const erase of [0, 1]) {
