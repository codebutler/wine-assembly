...
//   3. `i32.ge_u ... (i32.const N)` guard in src/04-cache.wat's
//      (dispatch-next) macro, which every handler expands
...
// The guard lives in the (dispatch-next) macro body, on its $nx_fn local.
const guardMatch = cache.match(/i32\.ge_u \(local\.get \$nx_fn\) \(i32\.const (\d+)\)\)[\s\S]{0,240}(?:0xCAC4BAD0|return_call \$dispatch_bad)/);
