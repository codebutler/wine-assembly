...
**Status: LANDED 2026-09-19** as the `(dispatch-next)` macro in
`src/04-cache.wat`, expanded at all 419 tail sites, each expanding function
declaring `$nx_fn`/`$nx_op` (the `9450d792` legacy-compiler transform below
was retired with that compiler; an in-memory JS transform, `46215810`, was
the measurement vehicle and was replaced by the macro the same day, byte-
identical modulo one label-less `block` per site). Measured on the quiet
box, gameplay windows, 3 reps, `perf stat` counters: StarCraft **−4.86% CPU
against a 1.13% null band**, Heroes II **−4.37% against 3.06%**; branches
−8%/−5.5%, instructions −6%/−4.5%, branch-miss rate 0.24% → 0.26%. So §2's
prediction argument did not hold — the shared site was already well
predicted — and §4's call-overhead argument is the whole win. §5's split of
the fat tail (cold `$dispatch_bad`, no hist check in the copies) is still
unbuilt: the landed copies inline the full body.
