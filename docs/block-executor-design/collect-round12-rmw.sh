#!/bin/bash
# Section 19 (round 12, lever C): what the read-modify-write STORE split buys,
# over the same 13 windows section 16.6 measured the round-11 load split on.
#
# `off` is --no-block-exec-rmw, which restores round 11's behaviour -- every
# `[m] OP= reg`, `[m] OP= imm`, `inc/dec/not/neg [m]` a whole-instruction
# fallback; `on` is the default. THE COLUMN TO READ IS `fallback`, not any uop
# count: the product of this lever is fewer trips out of the executor, and
# splitting one instruction into three micro-ops makes the uop count go UP.
#
